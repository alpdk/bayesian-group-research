  ...
  def generate(self, model, *, num_samples, num_steps, eps, inject_bos,
    ...
    if prefix is not None:
      for i, prompt in enumerate(prefix):
        tokens = prompt.to(device=device, dtype=xt.dtype).flatten()
        if inject_bos and bos_id is not None and (
            tokens.numel() == 0 or tokens[0] != bos_id):
          tokens = torch.cat(
            [torch.tensor([bos_id], device=device, dtype=xt.dtype), tokens])
        length = min(int(tokens.numel()), max_length)
        xt[i, :length] = tokens[:length]
        prefix_len[i] = length
    elif inject_bos:
      xt[:, 0] = bos_id
      prefix_len[:] = 1
    ...
    for step_idx in range(num_steps):
      ...
      if step_idx != num_steps - 1:
        ...
        # Only insert at valid gaps (before current length); never inside prefix
        gaps = torch.arange(max_length + 1, device=device).view(1, -1)
        ext = ext * (gaps <= xt_len.view(batch_size, 1)).long()
        ext = ext * (gaps >= prefix_len.view(batch_size, 1)).long()
