#!/usr/bin/env bash
# TinyGSM overfit: 2 packed examples, 4000 optimizer steps, 1 GPU.
# Train and generate+score on the same rows (pass@1_k1).
#
#   METHODS="mdlm flexmdm puma editflow" bash scripts/run_overfit.sh
set -euo pipefail
...
LOG_DIR="${LOG_DIR:-$REPO/outputs/overfit}"
...
mkdir -p "$LOG_DIR"
echo "RUN_MARKER overfit_start host=$(hostname) iso=$(date --iso-8601=seconds) py=$PY cuda=$($PY -c 'import torch; print(torch.cuda.is_available(), torch.cuda.device_count() if torch.cuda.is_available() else 0)')"

run_mdlm() {
  echo "RUN_MARKER method=mdlm start iso=$(date --iso-8601=seconds)"
  cd "$REPO/UNI-D2"
  export PYTHONPATH="$REPO/UNI-D2/src"
  mapfile -t OVERRIDES < <("$PY" "$REPO/scripts/hydra_from_yaml.py" "$REPO/configs/overfit/mdlm_tinygsm.yaml")
...
run_flexmdm() {
  ...
  cd "$REPO/UNI-D2"
  export PYTHONPATH="$REPO/UNI-D2/src"
  mapfile -t OVERRIDES < <("$PY" "$REPO/scripts/hydra_from_yaml.py" "$REPO/configs/overfit/flexmdm_tinygsm.yaml")
  "$PY" -u -m discrete_diffusion "${OVERRIDES[@]}" || return 1
...
run_puma() {
  ...
  if [ ! -s "$DATA_DIR/meta.json" ]; then
    ...
    "$PY" data/tiny_gsm.py --out_dir "$DATA_DIR" --max_len 256 --limit 2 --num_proc 1 || return 1
  fi
  "$PY" train.py --cfg "$REPO/configs/overfit/puma_tinygsm.yaml" || return 1
...
run_editflow() {
  ...
  unset PYTHONPATH
  SAVE_DIR="${SAVE_DIR_EDITFLOW:-$REPO/outputs/overfit/editflow-tinygsm-2}"
  mkdir -p "$SAVE_DIR"
  "$PY" train.py --cfg "$REPO/configs/overfit/editflow_tinygsm.yaml" --save-dir "$SAVE_DIR" || return 1
...
echo "RUN_MARKER overfit_end iso=$(date --iso-8601=seconds) failed=$failed"
exit "$failed"
