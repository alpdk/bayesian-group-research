...
def train(config):
  ...
  trainer_kwargs = omegaconf.OmegaConf.to_container(config.trainer, resolve=True)
  if not isinstance(trainer_kwargs, dict):
    raise TypeError(f"config.trainer must be a mapping, got {type(trainer_kwargs)}")
  # Lightning Trainer rejects unknown kwargs. Accept trainer.fault_tolerant as
  # a Hydra alias of training.fault_tolerant (cluster scripts used the former).
  fault_tolerant = bool(config.training.get('fault_tolerant', False))
  if 'fault_tolerant' in trainer_kwargs:
    fault_tolerant = bool(trainer_kwargs.pop('fault_tolerant'))
  if fault_tolerant:
