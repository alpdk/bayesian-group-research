#!/usr/bin/env bash
# TinyGSM 100k, L=256, global batch 64, 260k steps.
# In-domain TinyGSM exec + GSM8K transfer (PUMA-style Python vs #### gold).
set -euo pipefail
...
python -u -m discrete_diffusion \
    ...
    wandb.name=mdlm-tinygsm-L256-shared-params \
    wandb.group=shared-params \
    wandb.tags=[tinygsm,mdlm,shared-params,L256] \
    ...
    hydra.run.dir=./outputs/tinygsm/mdlm-shared-params \
    "$@"
