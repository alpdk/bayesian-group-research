...
class AbsorbingSampler(Sampler):
  ...
  def _apply_prefix(self, model, x, prefix, inject_bos):
    """Freeze prompt tokens; the rest stay masked (absorbing prior)."""
    bos_id = model.tokenizer.bos_token_id
    for i, prompt in enumerate(prefix):
      tokens = prompt.to(device=x.device, dtype=x.dtype).flatten()
      if inject_bos and (tokens.numel() == 0 or tokens[0] != bos_id):
        tokens = torch.cat(
          [torch.tensor([bos_id], device=x.device, dtype=x.dtype), tokens])
      length = min(int(tokens.numel()), x.shape[1])
      x[i, :length] = tokens[:length]
    return x
