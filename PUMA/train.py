...
def main(cfg: DictConfig, resume_path: str = None):
    ...
    # resume from checkpoint (weights; optimizer/scheduler if present)
    global_step = 0
    start_epoch = 0
    if resume_path is not None:
        ...
        if train_cfg.ema is not None:
            ...
            if ema_state is None:
                ema_sib = os.path.join(
                    os.path.dirname(resume_path), f"ema_step={global_step}.pt")
                if os.path.isfile(ema_sib):
                    ema_state = torch.load(ema_sib, map_location="cpu").get("ema_state_dict")
                    if is_main and ema_state is not None:
                        print(f"Loaded EMA from {ema_sib}")
            if ema_state is not None:
                ema.load_state_dict(ema_state)
                ema.move_shadow_params_to_device(device)
        if strategy == "progressive":
            while next_k_idx < len(k_schedule) and global_step >= k_schedule[next_k_idx][1]:
    ...
    # early stopping on validation loss (val_loss is all-reduced, identical on every rank)
    es_cfg = train_cfg.get("early_stopping", None)
    es_enabled = es_cfg is not None and bool(es_cfg.get("enabled", True))
    if es_enabled:
        ...
        es_bad_evals = 0
        if is_main:
            print(f"Early stopping enabled: patience={es_patience} evals, min_delta={es_min_delta}")
    stop_training = False
    ...
    for epoch in range(start_epoch, train_cfg.num_epochs):
        ...
        for itr in pbar:
            ...
            if max_steps > 0 and global_step >= max_steps:
                stop_training = True
                if is_main:
                    print(f"[MAX-STEPS] Stopping at step {global_step}")
            ...
            if is_main:
                ...
                if global_step % train_cfg.logging_steps == 0:
                    ...
                    if cfg.wandb.wandb:
                        ...
                        n_tok = 0
                        if strategy == "progressive":
                            n_tok = int(pool.x0.numel()) * world_size
                        elif strategy in ("standard", "arm"):
                            n_tok = int(input_ids.numel()) * world_size
                        payload = {
                            "train/loss": float(loss.item()),
                            "healthy/grad_norm": grad_norm(model.parameters()),
                        ...
                        if strategy == "progressive":
                            payload["stats/current_k"] = current_k
            ...
            if global_step % train_cfg.eval_steps == 0:
                ...
                # early stopping bookkeeping (same decision on all ranks, no broadcast needed)
                if es_enabled:
                    improved = val_loss < es_best - es_min_delta
                    if improved:
                        es_best = val_loss
                        es_bad_evals = 0
                    else:
                    ...
                    if is_main:
                        if improved:
                            ...
                            for old in os.listdir(best_dir):
                                os.remove(os.path.join(best_dir, old))
                            if train_cfg.ema is not None:
                                best_path = save_ema_snapshot(best_dir, model, ema, cfg, epoch, global_step, val_loss, val_acc_dict)
                            else:
                                best_path = save_model_snapshot(best_dir, model, cfg, epoch, global_step, val_loss=val_loss, extra=val_acc_dict)
                            print(f"[EARLY-STOP] New best val_loss={val_loss:.6f}, saved: {best_path}")
                        else:
                            print(f"[EARLY-STOP] No improvement ({es_bad_evals}/{es_patience}), best={es_best:.6f}, current={val_loss:.6f}")

                    if es_bad_evals >= es_patience:
                        stop_training = True
                        if is_main:
                            print(f"[EARLY-STOP] Stopping at step {global_step}: no val_loss improvement in {es_patience} evals (best={es_best:.6f})")
