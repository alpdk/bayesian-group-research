#!/usr/bin/env bash
# APPS, planner L=512, global batch 64.
set -euo pipefail
...
DATA_DIR="${DATA_DIR:-$REPO_ROOT/data/apps_shared_params}"
SCRATCH="${DISCRETE_DIFFUSION_SCRATCH_DIR:-$HOME/.cache/discrete_diffusion}"
SLIM="$SCRATCH/apps/apps_raw.dat"
NPROC="${NPROC_PER_NODE:-1}"
GLOBAL_BS="${GLOBAL_BS:-64}"
PER_GPU_BS="${PER_GPU_BS:-$((GLOBAL_BS / NPROC))}"

if [ ! -d "$SLIM" ] && [ ! -f "$SLIM/dataset_dict.json" ]; then
  python -c "from discrete_diffusion.data.loaders import load_code_qa_dataset; load_code_qa_dataset('apps', '$SCRATCH/apps')"
fi
...
if [ "$NPROC" -eq 1 ]; then
  python train.py --cfg yaml_files/apps_shared_params.yaml \
    data.data_dir="$DATA_DIR" \
    data.training.per_gpu_batch_size="$PER_GPU_BS" \
    training.batch_size="$PER_GPU_BS" \
    wandb.project="$WANDB_PROJECT" \
    wandb.name="${WANDB_NAME:-latentmdm-apps-L512-shared-params}" \
    wandb.entity=null \
    wandb.wandb=true
else
  python -m torch.distributed.run --standalone --nnodes=1 --nproc_per_node="$NPROC" \
    train.py --cfg yaml_files/apps_shared_params.yaml \
    data.data_dir="$DATA_DIR" \
    data.training.per_gpu_batch_size="$PER_GPU_BS" \
    training.batch_size="$PER_GPU_BS" \
    wandb.project="$WANDB_PROJECT" \
