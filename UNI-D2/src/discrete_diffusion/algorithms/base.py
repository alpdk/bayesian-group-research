...
class TrainerBase(L.LightningModule):
  ...
  def on_train_start(self):
    ...
    for cfg in getattr(self.trainer, "lr_scheduler_configs", []) or []:
      sch = getattr(cfg, "scheduler", None)
      if sch is None or not hasattr(sch, "t_initial"):
        continue
      warmup = int(getattr(sch, "warmup_t", 0) or 0)
      sch.t_initial = max(max_steps - warmup, 1)
      if hasattr(sch, "step_update"):
        sch.step_update(num_updates=step)
      elif hasattr(sch, "_last_epoch"):
        sch._last_epoch = step
  ...
  def training_step(self, batch, batch_idx):
    ...
    self.log(name='train/loss', value=losses.loss, on_step=True, on_epoch=False, sync_dist=True, prog_bar=True)
  ...
  def on_before_optimizer_step(self, optimizer, *args, **kwargs):
    grads = [p.grad.detach().norm(2) ** 2 for p in self.parameters() if p.grad is not None]
    if grads:
      self.log(
        'healthy/grad_norm', torch.sqrt(sum(grads)),
