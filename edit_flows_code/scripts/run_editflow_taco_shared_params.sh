#!/usr/bin/env bash
# TACO, prompt/target 512+512 bytes, global batch 64, 20k steps, hidden-test eval.
set -euo pipefail
...
cd "$ROOT"
SAVE_DIR="${SAVE_DIR:-$ROOT/results/editflow-taco-L512-shared-params}"
mkdir -p "$SAVE_DIR"
"$PY" train.py \
  --dataset taco \
  --steps 20000 \
  ...
  --wandb-name editflow-taco-L512-shared-params \
  --save-dir "$SAVE_DIR" \
  --save-every 2000 \
  --log-every 50 \
  --seed 42 \
